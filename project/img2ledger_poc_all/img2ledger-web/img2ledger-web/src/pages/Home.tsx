...
export default function Home() {
  const [apiBase, setApiBase] = useState(() => {
    // Vite env override: VITE_API_BASE=http://localhost:8000
    // fallback to current origin (when same-domain reverse proxy)
              ...
              {!result ? (
                <div className="space-y-3">
                  ...
                  <div className="text-xs text-muted-foreground leading-relaxed">
                    上传成功后，服务将返回一个 <span className="font-mono">job_id</span>，并提供两个 Excel：
                    <br />
                    <span className="font-mono">business.xlsx</span>（业务字段）与 <span className="font-mono">review.xlsx</span>（含审计列）。
                  </div>
                </div>
              ) : (
